...
//    //printf("[checkcachecounter]@@@VPU in3 \n")
  ...
  //////////////////////////////////////////////////////////////////////Adder////////////////////////////////////////////////////////////////////////////////////
  val Adders = Vec(Seq.fill(8){Module( new ValExec_MulAddRecF16_add()).io })//fill(8)
  val adder_out = Wire(Vec(8, UInt(16.W)))
  val adder_out_cat = Wire(UInt(128.W))
  val reg_sixth = Reg(UInt(16.W))
  reg_sixth := adder_out(6)

// wire up the ports of the adders
 when (!reduction_sum){
  for(i <- 0 until n) {
     Adders(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     Adders(i).b := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0) 
   ...
   for(i <- 0 until 4) {
     ...
     Adders(7).a  := adder_out(6)
     Adders(7).b  := reg_sixth
     adder_out(7) := Adders(7).result


 }

   adder_out_cat := Cat(adder_out(7),adder_out(6),adder_out(5),adder_out(4),adder_out(3),adder_out(2),adder_out(1),adder_out(0))
  ...
  val redsum_out_cat_sec = Cat(adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7),adder_out(7))
  val Reg_redsum_out = Reg(UInt(256.W))
   Reg_redsum_out := Mux(counter_num === UInt(0), redsum_out_cat, redsum_out_cat_sec)

    //printf("[checkcachecounter]**********add a [%x %x %x %x %x %x %x %x] b [%x %x %x %x %x %x %x %x] out [%x %x %x %x %x %x %x %x] \n ", Adders(7).a, Adders(6).a, Adders(5).a, Adders(4).a, Adders(3).a, Adders(2).a, Adders(1).a, Adders(0).a, Adders(7).b, Adders(6).b, Adders(5).b, Adders(4).b, Adders(3).b, Adders(2).b, Adders(1).b, Adders(0).b,adder_out(7),adder_out(6),adder_out(5),adder_out(4),adder_out(3),adder_out(2),adder_out(1),adder_out(0))
  ...
  //////////////////////////////////////////////////////////////////////multipler////////////////////////////////////////////////////////////////////////////////////
   val Multipliers = Vec(Seq.fill(8){Module( new ValExec_MulAddRecF16_mul()).io })
   val multiplier_out = Wire(Vec(8, UInt(16.W)))
   val multiplier_out_cat = Wire(UInt(128.W))

   // wire up the ports of the adders
   for(i <- 0 until n) {
     Multipliers(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     Multipliers(i).b := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0) 
     multiplier_out(i) := Multipliers(i).result
   }

   multiplier_out_cat := Cat(multiplier_out(7),multiplier_out(6),multiplier_out(5),multiplier_out(4),multiplier_out(3),multiplier_out(2),multiplier_out(1),multiplier_out(0))
...
//     //printf("[checkcachecounter]invpu1_multiply a [%x %x %x %x %x %x %x %x] b [%x %x %x %x %x %x %x %x] out [%x %x %x %x %x %x %x %x] \n ", Multipliers(7).a, Multipliers(6).a, Multipliers(5).a, Multipliers(4).a, Multipliers(3).a, Multipliers(2).a, Multipliers(1).a, Multipliers(0).a, Multipliers(7).b, Multipliers(6).b, Multipliers(5).b, Multipliers(4).b, Multipliers(3).b, Multipliers(2).b, Multipliers(1).b, Multipliers(0).b,multiplier_out(7),multiplier_out(6),multiplier_out(5),multiplier_out(4),multiplier_out(3),multiplier_out(2),multiplier_out(1),multiplier_out(0))
  ...
  ////////////////////////////////////////////////VFMADD//////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   val MulAdds = Vec(Seq.fill(8){Module( new ValExec_MulAddRecF16()).io })
   val MulAdd_out = Wire(Vec(8, UInt(16.W)))
   val MulAdd_out_cat = Wire(UInt(128.W))

   // wire up the ports of the adders
   for(i <- 0 until n) {
     MulAdds(i).a  := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     MulAdds(i).b  := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     MulAdds(i).c  := (((io.req.bits.in3 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     MulAdds(i).roundingMode := UInt(0)
     MulAdds(i).detectTininess := UInt(1)  
     MulAdd_out(i) := MulAdds(i).result
    }

   MulAdd_out_cat := Cat(MulAdd_out(7),MulAdd_out(6),MulAdd_out(5),MulAdd_out(4),MulAdd_out(3),MulAdd_out(2),MulAdd_out(1),MulAdd_out(0))
  ...
  when (io.req.bits.fn === ALU.FN_VFMADD){
     //printf("[checkcachecounter]********************invpu1 mult_add a [%x %x %x %x %x %x %x %x] b [%x %x %x %x %x %x %x %x]  c [%x %x %x %x %x %x %x %x] out [%x %x %x %x %x %x %x %x] \n ", MulAdds(7).a, MulAdds(6).a, MulAdds(5).a, MulAdds(4).a, MulAdds(3).a, MulAdds(2).a, MulAdds(1).a, MulAdds(0).a,MulAdds(7).b, MulAdds(6).b, MulAdds(5).b, MulAdds(4).b, MulAdds(3).b, MulAdds(2).b, MulAdds(1).b, MulAdds(0).b,MulAdds(7).c, MulAdds(6).c, MulAdds(5).c, MulAdds(4).c, MulAdds(3).c, MulAdds(2).c, MulAdds(1).c, MulAdds(0).c,MulAdd_out(7),MulAdd_out(6),MulAdd_out(5),MulAdd_out(4),MulAdd_out(3),MulAdd_out(2),MulAdd_out(1),MulAdd_out(0))
  }

  //////////////////////////////////////////////////////////////////////comparator////////////////////////////////////////////////////////////////////////////////////
   val comparators_lt = Vec(Seq.fill(8){Module( new ValExec_CompareRecF16_lt()).io })
   val comparator_lt_out = Wire(Vec(8, UInt(16.W)))
   val comparator_lt_out_cat = Wire(UInt(128.W))

   // wire up the ports of the adders
   for(i <- 0 until n) {
     comparators_lt(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     comparators_lt(i).b := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0) 
     comparator_lt_out(i) := comparators_lt(i).actual.out
    }

    comparator_lt_out_cat := Cat(comparator_lt_out(7),comparator_lt_out(6),comparator_lt_out(5),comparator_lt_out(4),comparator_lt_out(3),comparator_lt_out(2),comparator_lt_out(1),comparator_lt_out(0))
   ...
   val comparators_le = Vec(Seq.fill(8){Module( new ValExec_CompareRecF16_le()).io })
   val comparator_le_out = Wire(Vec(8, UInt(16.W)))
   val comparator_le_out_cat = Wire(UInt(128.W))

   // wire up the ports of the adders
   for(i <- 0 until n) {
     comparators_le(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     comparators_le(i).b := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0) 
     comparator_le_out(i) := comparators_le(i).actual.out
    }

   comparator_le_out_cat := Cat(comparator_le_out(7),comparator_le_out(6),comparator_le_out(5),comparator_le_out(4),comparator_le_out(3),comparator_le_out(2),comparator_le_out(1),comparator_le_out(0))
   ...
   val comparators_eq = Vec(Seq.fill(8){Module( new ValExec_CompareRecF16_eq()).io })
   val comparator_eq_out = Wire(Vec(8, UInt(16.W)))
   val comparator_eq_out_cat = Wire(UInt(128.W))

   // wire up the ports of the adders
   for(i <- 0 until n) {
     comparators_eq(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)
     comparators_eq(i).b := (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0) 
     comparator_eq_out(i) := comparators_eq(i).actual.out
    }

   comparator_eq_out_cat := Cat(comparator_eq_out(7),comparator_eq_out(6),comparator_eq_out(5),comparator_eq_out(4),comparator_eq_out(3),comparator_eq_out(2),comparator_eq_out(1),comparator_eq_out(0))
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  val min_out = Wire(Vec(8, UInt(16.W)))
  val min_out_cat = Wire(UInt(128.W))

  val max_out = Wire(Vec(8, UInt(16.W)))
  val max_out_cat = Wire(UInt(128.W))

  val lt_bool_val = Wire(Vec(8, Bool()))
  val eq_bool_val = Wire(Vec(8, Bool()))
  val le_bool_val = Wire(Vec(8, Bool()))
  for(i <- 0 until n) {
   lt_bool_val(i) := comparator_lt_out(i) === UInt(1)
   eq_bool_val(i) := comparator_eq_out(i) === UInt(1)
   min_out(i) := Mux(lt_bool_val(i), (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0), Mux(eq_bool_val(i), (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0), (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0)))
  }

  for(i <- 0 until n) {
   le_bool_val(i) := comparator_le_out(i) === UInt(1)
   max_out(i) := Mux(le_bool_val(i), (((io.req.bits.in2 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0), (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 16*i)(15,0))
 }

  min_out_cat := Cat(min_out(7), min_out(6), min_out(5), min_out(4), min_out(3), min_out(2), min_out(1), min_out(0))

  val Reg_min_out = Reg(UInt(256.W))
  Reg_min_out := Mux(counter_num === UInt(0), min_out_cat, Cat(min_out_cat, Reg_min_out(127,0)))


  max_out_cat := Cat(max_out(7), max_out(6), max_out(5), max_out(4), max_out(3), max_out(2), max_out(1), max_out(0))
