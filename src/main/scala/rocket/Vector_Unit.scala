...
//  //printf("[checkcachecounter]VVVVVVVVEEEEEEEEECCCCCCCCCCTTTTTTTTOOOOOOOORRRRRRRR io.cpu.req.bits.number_of_elements %x \n", io.req.bits.number_of_elements)
  ...
  val reg_second = Reg(UInt(32.W))
  reg_second := adder_out(2)
 ...
 when (!reduction_sum){
  for(i <- 0 until n) {
   ...
   for(i <- 0 until 2) {
     Adders(i).a := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 32*(2*i))(31,0)
     Adders(i).b := (((io.req.bits.in1 >> counter_num * UInt(128))(127,0)) >> 32*(2*i+1))(31,0) 
     adder_out(i) := Adders(i).result
   }
 
     Adders(2).a := adder_out(0)
     Adders(2).b := adder_out(1)
     adder_out(2) := Adders(2).result

     Adders(3).a  := adder_out(2)
     Adders(3).b  := reg_second
     adder_out(3) := Adders(3).result
  ...
  val redsum_out_cat = Cat(adder_out(2),adder_out(2),adder_out(2),adder_out(2),adder_out(2),adder_out(2),adder_out(2),adder_out(2)) //or reg_second
  val redsum_out_cat_sec = Cat(adder_out(3),adder_out(3),adder_out(3),adder_out(3),adder_out(3),adder_out(3),adder_out(3),adder_out(3))
